...
const connectDB = async () => {
    try {
        ...
        // FIXED: Check if the MONGODB_URI already contains the database name
        let connectionString = process.env.MONGODB_URI;
        
        // If the URI doesn't end with the database name, append it
        // But also check if it already has a database name (doesn't end with '/')
        if (!connectionString.endsWith(DB_NAME) && !connectionString.includes('/' + DB_NAME)) {
