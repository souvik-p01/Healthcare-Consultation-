...
const PatientPortal = () => {
  const { user: authUser, logoutUser, apiCall, token } = useAppContext();
  ...
  const user = {
    name: authUser ? `${authUser.firstName || ''} ${authUser.lastName || ''}`.trim() || 'Patient' : 'Patient',
    age: authUser?.age || '--',
  ...
  // ✅ FIXED: Fetch patient data with proper error handling
  const fetchPatientData = useCallback(async () => {
    ...
    try {
      ...
      try {
        const notifResponse = await apiCall('/patients/notifications');
        if (notifResponse?.success && notifResponse?.data) {
          const formattedNotifications = notifResponse.data.map(n => ({
            id: n._id,
        ...
        if (error.response?.status === 404) {
          ...
          console.log('Unauthorized, skipping notifications');
        } else {
          toast.warning('Unable to load notifications');
        }
      }

      // Fetch appointments with error handling
      try {
        const apptResponse = await apiCall('/patients/appointments');
        ...
        if (error.response?.status === 404) {
          ...
          console.log('Unauthorized, skipping appointments');
        } else {
          toast.warning('Unable to load appointments');
        }
      }
    } catch (error) {
  ...
  // Health metrics (static for now, could be dynamic)
  const healthMetrics = [
  ...
  const upcomingAppointments = appointments.length > 0 ? appointments.map(apt => ({
    id: apt._id,
    doctor: apt.doctorName || apt.doctor?.name || 'Doctor',
    specialty: apt.specialty || 'General',
    date: new Date(apt.date).toLocaleDateString(),
    time: `${apt.startTime || '10:00'} - ${apt.endTime || '10:30'}`,
    type: apt.type || 'In-Person',
    status: apt.status || 'confirmed',
    avatar: (apt.doctorName || apt.doctor?.name || 'Dr').split(' ').map(n => n[0]).join(''),
  ...
  // Sidebar and responsive handlers
  useEffect(() => {
    const handleResize = () => {
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          <button 
            onClick={() => toast.info('Detailed health metrics coming soon!')}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-lg shadow p-4 md:p-6">
            ...
            <div className="space-y-3">
              {upcomingAppointments.slice(0, 2).map((apt) => (
                <div 
                  key={apt.id}
                  onClick={() => toast.info(`Appointment details:\nDoctor: ${apt.doctor}\nTime: ${apt.date} ${apt.time}\nType: ${apt.type}`)}
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow p-4 md:p-6 text-white">
            ...
            <button 
              onClick={() => toast.info('More health tips coming soon!')}
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-4 md:p-6 lg:p-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-4 mb-5 md:mb-6">
          ...
          <button 
            onClick={() => toast.info('Book appointment feature coming soon!')}
                      ...
                      <tr key={apt.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 md:px-4 md:py-3 whitespace-nowrap">
                          <div className="flex items-center gap-1 md:gap-2">
                            <button 
                              onClick={() => toast.info(`Viewing appointment with ${apt.doctor}`)}
          ...
          {[
            ...
            <div key={record.id} className="bg-gray-50 rounded-lg p-4 md:p-5 border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <div className="flex items-center gap-2">
                <button 
                  onClick={() => toast.info(`Viewing ${record.name}`)}
                ...
                <button 
                  onClick={() => toast.info(`Downloading ${record.name}`)}
